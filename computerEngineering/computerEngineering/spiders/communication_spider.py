...
class PubSpider(Spider):
   name = "communicationpubs"
   allowed_domains = ["www.joconline.com.cn"]
   start_urls = []
   
   basic_url = "http://www.joconline.com.cn/ch/reader/view_abstract.aspx?file_no="
   ...
   def parse(self, response):
     	...
     	#print response	
   	doi = ''.join(response.xpath('//span[@id="DOI"]/text()').extract())
  	...
  	title_English = ''.join(response.xpath('//span[@id="EnTitle"]/text()').extract())      
   	...
   	download_urls = "http://www.joconline.com.cn/ch/reader/create_pdf.aspx?file_no=" + fileno + "&flag=1&journal_id=txxb"      
